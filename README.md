...
| v3.0 | 2026-05-22 | 颜色选择扩展为 141 色弹窗；新增格式四时间戳解析；每 PC 独立阴影颜色/方向设置（BBCode `[shadow]`）；全局预览字体大小调节；昵称置灰默认开启；输入框支持图片粘贴/插入并保留至分析输出；新增「复制原文」「📎 插入图片」「🔍 全局替换」工具栏按钮；修复 Windows 剪贴板 `\r\n` 导致分析多余空行的问题；移除论坛分享功能；移除第三方追踪代码（两重改） |
